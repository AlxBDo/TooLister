<template>
  <div class="flex items-center justify-between">
    <h1 class="text-3xl my-4">ShoppingItem List</h1>

    <nuxt-link :to="{ name: 'shoppingitems-create' }"
      ...
      <tbody>
        <tr v-for="item in items" :key="item['@id']" class="border-b">
          <td class="px-6 py-4 text-sm">
            <nuxt-link :to="{ name: 'shoppingitems-id', params: { id: getIdFromIri(item['@id']) } }"
          ...
          <td class="px-6 py-4 text-sm">
            <nuxt-link :to="{ name: 'shoppingitems-id-edit', params: { id: getIdFromIri(item['@id']) } }"
              class="px-6 py-2 bg-green-600 text-white text-xs rounded shadow-md hover:bg-green-700">
              Edit
            </nuxt-link>
          </td>
  ...
  <div v-if="view" class="flex justify-center">
    <nav aria-label="Page navigation">
      <ul class="flex list-style-none">
        ...
        <li :class="{ disabled: !pagination.next }">
          <nuxt-link :to="{ name: 'shoppingitems-page-page', params: { page: pagination.last } }" :class="!pagination.next
              ? 'text-gray-500 pointer-events-none'
              : 'text-gray-800 hover:bg-gray-200'
...
import { useShoppingItemListStore } from "~~/stores/shoppingitem/list";
...
const shoppingitemListStore = useShoppingItemListStore();
const { items, view, error, isLoading, hubUrl } = await useFetchList<ShoppingItem>(
  "shopping_items"
);
shoppingitemListStore.setData({ items, view, error, isLoading, hubUrl });
...
useMercureList({ store: shoppingitemListStore, deleteStore: shoppingitemDeleteStore });

onBeforeUnmount(() => {
  shoppingitemListStore.$reset();
